...
class QLearningAgent(ReinforcementAgent):
  ...
  def update(self, state, action, nextState, reward):
    ...
    for a in self.getLegalActions(nextState):
      if max < self.Q_values[nextState, a]:
        max = self.Q_values[nextState, a]
    if (max != -float("inf")):
      self.Q_values[state, action] = ( (1 - self.alpha)*self.Q_values[state, action]+self.alpha*(reward+self.discount*max) )
...
class ApproximateQAgent(PacmanQAgent):
  ...
  def getQValue(self, state, action):
    ...
    sum = 0
    for i in f:
      sum += f[i]*self.weights[i]
    self.Q_values[state, action] = sum
    return sum
  ...
  def update(self, state, action, nextState, reward):
    ...
    if len(self.getLegalActions(nextState)) == 0:
      ...
      for i in f:
        self.weights[i] = self.weights[i] +self.alpha*correction*f[i]
      return
