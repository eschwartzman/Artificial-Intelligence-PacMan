    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if ((nextx, nexty) in self.corners):
                flags[self.corners.index((nextx, nexty))] = True


            if  hitsWall == False:
                successor = (((nextx, nexty), tuple(flags)), action, 1)
...
def cornersHeuristic(state, problem):
    ...
    for corner in corners:
        dx, dy = corner
        if flags[corners.index(corner)] == False:
            cornerDistance_fromState.append((abs(x - dx) + abs(y - dy)))

    return max(cornerDistance_fromState)
...
class AnyFoodSearchProblem(PositionSearchProblem):
    ...
    def isGoalState(self, state):
        ...
        for food in (self.food).asList():
            distance.append((abs(x - food[0]) + abs(y - food[1])))
        for food in (self.food).asList():
            if ((abs(x - food[0]) + abs(y - food[1]))) == min(distance):
                close = food
        if state == close:
            return True
        return False
