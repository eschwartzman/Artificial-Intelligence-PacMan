...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    for ghost in livingGhostPositionDistributions:
        mostLikely = 0
        ghostPos = ghost[0]
        for pos, prob in ghost.iteritems():
          if prob > mostLikely:
            mostLikely= prob
            ghostPos= pos
        distanceToGhost = self.distancer.getDistance(ghostPos, pacmanPosition)
    ...
    for a in gameState.getLegalPacmanActions():
      ...
      if self.distancer.getDistance(successorPosition, closestGhost) < minimum:
        minimum = self.distancer.getDistance(successorPosition, closestGhost)
        save_action = a
    return save_action
