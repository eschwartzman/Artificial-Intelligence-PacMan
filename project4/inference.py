...
class ParticleFilter(InferenceModule):
  ...
  def observe(self, observation, gameState):
    ...
    if noisyDistance == None:
        self.particles= util.Counter()
        self.partciles[self.getJailPosition()]= self.numParticles
        return

    #weighing
    for p in self.particles:
        trueDistance= util.manhattanDistance(p, pacmanPosition)
        self.particles[p]= self.particles[p] * emissionModel[trueDistance]

    #redistribute particles in case all weight are 0
    check= False
    for p in self.particles:
        if self.particles[p] != 0:
            check= True
            break
    if check== False:
        self.initializeUniformly(gameState)
    ...
    for i in range(0, self.numParticles):
        temp[util.sample(self.particles)] +=1
    self.particles= temp.copy()
  ...
  def elapseTime(self, gameState):
    ...
    "*** YOUR CODE HERE ***"
    temp= util.Counter()
    for oldPos, prob in self.particles.iteritems():
      newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
      for i in range (0, self.particles[oldPos]):
        temp[util.sample(newPosDist)] +=1
    self.particles= temp
...
class JointParticleFilter:
  ...
  def initializeParticles(self):
    ...
    for i in range(0, self.numParticles):
        ...
        for j in range (0, self.numGhosts):
            p= random.choice(self.legalPositions)
            newParticle.append(p)
        self.particles.append(tuple(newParticle))
  ...
  def elapseTime(self, gameState):
    ...
    for oldParticle in self.particles:
      ...
      "*** YOUR CODE HERE ***"
      #temp = list(newParticle)
      for i in range(self.numGhosts):
        #get distribution of the current ghost position
        newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, oldParticle), i, self.ghostAgents[i])

        #generate new ghost position base on the distribution
  ...
  def observeState(self, gameState):
    ...
    for particle in (self.particles):
        position_list= list(particle)
        weight=1
        for i, ghostPos in enumerate(position_list):
    ...
    #redistribute particles in case all weight are 0
    check= False
    for particle in weightedParticles:
        if weightedParticles[particle] !=0:
            check= True
            break
    if check== False:
        self.initializeParticles()
